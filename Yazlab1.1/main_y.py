...
def database_sil():
    ...
    cur.execute("delete from bilgisayar_index")
    con.commit()
...
def n_11_ucuza():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS bilgisayar_index (marka TEXT,model_no  TEXT,isletim_sistemi  TEXT,islemci_tipi 
    ...
    ekle = """INSERT INTO bilgisayar_index (marka,model_no,isletim_sistemi,islemci_tipi,islemci_nesli,ram,ekran_boyut,disk_tur,disk_boyut,fiyat,link,resim,site)  VALUES(
    ?,?,?,?,?,?,?,?,?,?,?,?,?) """
    for k in range(len(links)):
        cursor.execute(ekle, (
            markas[k], models[k], isletims[k], islemcis[k], islemci_models[k], rams[k], ekran[k], disktur[k], disks[k],
            prices[k], links[k], imgs[k], "Ucuza.com"))
        connection.commit()
...
def trendyol():
    ...
    for sayfa in range(1,8):
        ...
        for li in list:
         link = li.a.get("href")
         links.append(link)
         url_ic = 'https://www.trendyol.com' + link
         html_ic = requests.get(url_ic).content
         soup_ic = BeautifulSoup(html_ic, "html.parser")
...
def index():
    ...
    cur.execute("select * from bilgisayar_index")

    rows = cur.fetchall()
    return render_template("index.html", rows=rows)

@app.route("/view.html/artan.html")
def artan():
    con = sql.connect("firstDatabase.db")
    ...
    cur.execute("select * from bilgisayar order by cast(fiyat as int)")

    rows = cur.fetchall()
    return render_template("artan.html", rows=rows)

@app.route("/view.html/azalan.html")
def azalan():
    con = sql.connect("firstDatabase.db")
    ...
    cur.execute("select * from bilgisayar order by  cast(fiyat as int) desc")

    rows = cur.fetchall()
    return render_template("azalan.html", rows=rows)
